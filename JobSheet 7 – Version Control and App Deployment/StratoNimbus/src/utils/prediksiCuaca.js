...
// TTLs (dalam detik untuk cache wrapper)
const DEFAULT_TTL = 10 * 60; // 10 menit
const ERROR_TTL = 60; // 1 menit untuk cache error/negatif

const makeKey = (lat, lon) => `weather:${lat},${lon}`;
...
const forecast = async (latitude, longitude, callback, options = {}) => {
  ...
  const ttl = typeof options.ttl === 'number' ? Math.floor(options.ttl / 1000) : DEFAULT_TTL;
  const key = makeKey(latitude, longitude);

  // Cek cache lebih dahulu (async)
  try {
    const cached = await cacheGet(key);
    if (cached) {
      if (cached && cached.__error) return callback(cached.__error, undefined);
      return callback(undefined, cached);
    }
  } catch (e) {
    // Jika cache gagal, lanjutkan tanpa cache
  }
  ...
  if (WEATHERSTACK_KEY) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // simpan negative cache singkat
        try { await cacheSet(key, { __error: errMsg }, ERROR_TTL); } catch (e) {}
        try {
      ...
      if (data.error) {
        ...
        try { await cacheSet(key, { __error: info }, ERROR_TTL); } catch (e) {}
        try {
          const fb = await tryOpenMeteo();
          return callback(undefined, fb);
        } catch (e) {
      ...
      try { await cacheSet(key, prediksi, ttl); } catch (e) {}
      return callback(undefined, prediksi);
    } catch (e) {
      const errMsg = 'Tidak dapat terkoneksi ke layanan cuaca.';
      try { await cacheSet(key, { __error: errMsg }, ERROR_TTL); } catch (ee) {}
      try {
        const fb = await tryOpenMeteo();
        return callback(undefined, fb);
      } catch (ee) {
