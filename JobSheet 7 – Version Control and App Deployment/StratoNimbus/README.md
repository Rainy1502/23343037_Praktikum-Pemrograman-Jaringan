...
```
 ...
 Berikut langkah cepat untuk men-deploy aplikasi ini ke Vercel menggunakan serverless functions (direkomendasikan):
 
 1. Install Vercel CLI (opsional untuk deploy lewat terminal):
 ```powershell
 npm i -g vercel
 ```
 2. Login ke akun Vercel (interaktif):
 ```powershell
 vercel login
 ```
 3. Pastikan Anda sudah menambahkan environment variables di Dashboard Vercel (Project → Settings → Environment Variables) atau via CLI:
 ```powershell
 vercel env add MEDIASTACK_KEY production
 vercel env add WEATHERSTACK_KEY production
 vercel env add REDIS_URL production
 ```
 4. Deploy (pertama kali Vercel akan meminta konfirmasi project):
 ```powershell
 vercel --prod
 ```
 5. Untuk menjalankan fungsi secara lokal gunakan:
 ```powershell
 vercel dev
 ```
 
 Catatan:
 - File `api/index.js` telah dibuat untuk membungkus Express dengan `serverless-http` sehingga aplikasi dapat berjalan di Vercel.
 - `vercel.json` ada di root dan mengarahkan semua route ke `api/index.js`.
 - Pastikan `.env` tidak di-commit. Gunakan environment variables di Dashboard Vercel untuk production.
